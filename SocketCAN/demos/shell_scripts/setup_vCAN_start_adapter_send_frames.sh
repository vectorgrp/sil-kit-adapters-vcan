...
timeout 30s grep -q 'Press CTRL + C to stop the process...' <(tail -f /$SCRIPT_DIR/SilKitAdapterSocketCAN.out) || (echo "[error] Timeout reached while waiting for SilKitAdapterSocketCAN to start"; exit 1;)
